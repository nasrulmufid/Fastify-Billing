...
class MikrotikClient {
  ...
  // ========================================================
  // Hotspot Profile — /ip hotspot profile
  // ========================================================

  /** Cari hotspot profile berdasarkan name. Return id bila ada, null bila tidak. */
  private async findHotspotProfileId(name: string): Promise<string | null> {
    const rows = (await this.request<Array<{ ".id": string; name: string }>>(
      "GET",
      `/ip/hotspot/profile?name=${encodeURIComponent(name)}`,
    )) as Array<{ ".id": string; name: string }>
    return rows[0]?.[".id"] ?? null
  }

  /**
   * Pastikan hotspot profile ada di MikroTik.
   * Buat bila belum ada; update bila sudah ada.
   * Return nama profile.
   */
  async ensureHotspotProfile(opts: {
    name: string
    sharedUsers: number
    sessionTimeout: number
    rateLimit?: string
  }): Promise<string> {
    const { name, sharedUsers, sessionTimeout, rateLimit } = opts
    const existingId = await this.findHotspotProfileId(name)

    const payload: Record<string, unknown> = {
      name,
      "shared-users": String(sharedUsers),
      "session-timeout": sessionTimeout > 0 ? `${String(sessionTimeout)}s` : "00:00:00",
    }
    if (rateLimit) {
    ...
    if (existingId) {
      ...
      await this.updateResource("ip/hotspot/profile", existingId, payload)
    } else {
      // Buat profile baru
      await this.createResource("ip/hotspot/profile", payload)
    }
    return name
  }

  /** Sinkronisasi semua hotspot profile dari DB ke router. Return jumlah yang disync. */
  async syncHotspotProfiles(
    profiles: Array<{ name: string; sharedUsers: number; sessionTimeout: number; downloadSpeed: number; uploadSpeed: number }>,
  ): Promise<number> {
    let synced = 0
    for (const p of profiles) {
      const rateLimit = `${p.downloadSpeed}M/${p.uploadSpeed}M`
      await this.ensureHotspotProfile({
        name: p.name,
  ...
  /** Cari hotspot user berdasarkan username. Return id bila ada, null bila tidak. */
  private async findHotspotUserId(username: string): Promise<string | null> {
    const rows = (await this.request<Array<{ ".id": string }>>(
      "GET",
      `/ip/hotspot/user?user=${encodeURIComponent(username)}`,
    )) as Array<{ ".id": string }>
    return rows[0]?.[".id"] ?? null
  }

  /**
   * Pastikan hotspot user ada di MikroTik.
   * Buat bila belum ada; update bila sudah ada.
   */
  async ensureHotspotUser(opts: {
    username: string
    password: string
    profile: string
    service?: string
  }): Promise<void> {
    const { username, password, profile, service = "all" } = opts
    const existingId = await this.findHotspotUserId(username)
    ...
    if (existingId) {
      await this.updateResource("ip/hotspot/user", existingId, payload)
    } else {
      await this.createResource("ip/hotspot/user", payload)
    }
  }

  /** Sinkronisasi semua hotspot user dari DB ke router. Return jumlah yang disync. */
  async syncHotspotUsers(
    users: Array<{ username: string; password: string; profileName: string }>,
  ): Promise<number> {
    let synced = 0
    for (const u of users) {
      await this.ensureHotspotUser({
        username: u.username,
        password: u.password,
        profile: u.profileName || "default",
      })
      synced++
    }
    return synced
...
/**
 * Sinkronisasi hotspot profile dari DB ke MikroTik `/ip hotspot profile`.
 * Buat/update profile sesuai name, shared-users, session-timeout, rate-limit.
 */
export async function syncHotspotProfilesToRouter(
  app: FastifyInstance,
  routerId: number,
  profiles: Array<{ name: string; sharedUsers: number; sessionTimeout: number; downloadSpeed: number; uploadSpeed: number }>,
): Promise<RouterResult<number>> {
  return withRouter(app, routerId, async (client) => {
    ...
    for (const p of profiles) {
      const rateLimit = `${p.downloadSpeed}M/${p.uploadSpeed}M`
      await client.ensureHotspotProfile({
        name: p.name,
        sharedUsers: p.sharedUsers,
        sessionTimeout: p.sessionTimeout,
        rateLimit,
...
/**
 * Sinkronisasi hotspot user dari DB ke MikroTik `/ip hotspot user`.
 * Buat/update user sesuai username, password, profile.
 */
export async function syncHotspotUsersToRouter(
  app: FastifyInstance,
  routerId: number,
  users: Array<{ username: string; password: string; profileName: string }>,
): Promise<RouterResult<number>> {
  return withRouter(app, routerId, async (client) => {
    let synced = 0
    for (const u of users) {
      await client.ensureHotspotUser({
        username: u.username,
        password: u.password,
        profile: u.profileName || "default",
      })
